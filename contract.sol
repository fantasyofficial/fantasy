...
/**
    ...
    Decimal = 9
    8% of each transaction to the liquidity pool
    8% of each transaction to the BNB Pool
...
contract FANTASY is Context, IBEP20, Ownable {
    ...
    string private _symbol   = "FSY";
    uint8 private  _decimals = 9;
    
    uint256 public _taxFee       = 4; 
    uint256 public _liquidityFee = 16; 
    uint256 public _percentageOfLiquidityForBnbpool = 50; 
    ...
    event BnbpoolFeeSent(address to, uint256 bnbSent);
    ...
    constructor (address cOwner, address bnbpoolWallet) Ownable(cOwner) {
        _bnbpoolWallet = bnbpoolWallet;
        ...
        _isExcludedFromFee[_bnbpoolWallet]     = true;
    ...
    function setBnbpoolWallet(address bnbpoolWallet) external onlyOwner {
        _bnbpoolWallet = bnbpoolWallet;
    }
    ...
    function setPercentageOfLiquidityForBnb(uint256 bnbpoolFee) external onlyOwner {
        _percentageOfLiquidityForBnbpool = bnbpoolFee;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        // take bnbpool fee
        uint256 bnbpoolFee          = newBalance.mul(_percentageOfLiquidityForBnbpool).div(100);
        uint256 bnbForLiquidity = newBalance.sub(bnbpoolFee);
        if (bnbpoolFee > 0) {
            payable(_bnbpoolWallet).transfer(bnbpoolFee);
            emit BnbpoolFeeSent(_bnbpoolWallet, bnbpoolFee);
        }
